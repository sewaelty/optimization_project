...
def dryer(Time_interval,merged_data,model,duration_wm, wm_start):
    ...
    max_gap_wm_dryer = 2    # Maximum number of hours allowed between washing machine end and dryer start
    ...
    model.addConstrs((binary_dryer[t] <= merged_data['Allowed_dryer'][t] for t in range(len(binary_dryer))), name="allowed_dryer")
...
def PV_no_feed_in_and_penalty(Time_interval,merged_data,model,power_dishwasher,binary_dishwasher
